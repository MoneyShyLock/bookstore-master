    ...
    <div class="sign-in-wrapper">
        <div class="sign-in-inner">
            <div class="login-brand text-center">
                <i class="fa fa-database m-right-xs"></i> 书城 <strong class="text-skin">登录界面</strong>
            </div>

            <form>
                <div class="form-group m-bottom-md">
                    <input id="_name" name="name" type="text" class="form-control" placeholder="请输入账号">
                ...
                <div class="m-top-md p-top-sm">
                    <a href="protal" class="btn btn-success block" type="button">暂不登录</a>
                </div>
            </form>
        </div><!-- ./sign-in-inner -->
    </div><!-- ./sign-in-wrapper -->
    ...
    $('#_name').blur(function () {
        ...
        $.ajax({
            ...
            success: function(result) {                    //result为返回值
                console.log(result);
                if(result.success){
                    $('#admin_message').html("用户名不存在").css("color","red");
                    $('#_name')[0].focus();
                }else {
                    $('#admin_message').html("用户名可用").css("color","green");
                }
            }
        });
    })
